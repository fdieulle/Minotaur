...
namespace Minotaur.Codecs
{
    ...
    /// The  goal of this compression algorithm is to keep the minimum value and store them with leading zero compression.
    /// </summary>
    public unsafe class MinDeltaInt64Codec : ICodec<Int64Entry>
    {
        #region Implementation of ICodec2
        ...
        {
            return Codec.MAX_INT64_LENGTH * (count + 1)
                 + Codec.MAX_INT64_LENGTH * (count + 2);
        }

        public int Encode(Int64Entry* src, int count, byte* dst)
        {
            if (count <= 0) return count;

            var minDeltaTicks = long.MaxValue;
            var maxNegDeltaValue = long.MinValue;
            var minPosDeltaValue = long.MaxValue;
            ...
            Codec.EncodeInt64(src->value, ref dst);
            Codec.EncodeInt64(maxNegDeltaValue, ref dst);
            Codec.EncodeInt64(minPosDeltaValue, ref dst);
            ...
            for (var i = 1; i < count; i++)
            {
                ...
                var delta = (src + i)->value - (src + i - 1)->value;
                if (delta < 0) Codec.EncodeInt64(true, -delta - maxNegDeltaValue, ref dst);
                else Codec.EncodeInt64(false, delta - minPosDeltaValue, ref dst);
            }

            return (int)(dst - start);
        }

        public int Decode(byte* src, int len, Int64Entry* dst)
        {
            if (len <= 0) return len;
            ...
            dst->value = Codec.DecodeInt64(ref src);
            var maxNegDeltaValue = Codec.DecodeInt64(ref src);
            var minPosDeltaValue = Codec.DecodeInt64(ref src);
            ...
            while (src < end)
            {
                ...
                var delta = Codec.DecodeInt64(ref src, out var isNegative);
                if (isNegative) dst->value = (dst - 1)->value - (delta + maxNegDeltaValue);
                else dst->value = (dst - 1)->value + delta + minPosDeltaValue;
